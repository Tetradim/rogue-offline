  ...
  it('materializes the exact verified embedded asset set for the transactional installer', async () => {
    ...
    try {
      ...
      expect(await readFile(
        path.join(value.sourceRoot, 'assets', 'stage-1', 'sprite.png'),
        'utf8',
      )).toBe('sprite bytes')
    } finally {
    ...
    const cases = [
      ...
      },
      {
        ...
        value: {
          format: 'pokerogue-mod-package',
          schemaVersion: 1,
          manifest: manifest({ relativePath, sha256: hash }),
          assets: [],
        },
      ...
      },
      {
        ...
        value: {
          ...
          manifest: manifest({ relativePath, sha256: hash }),
          assets: [
            { relativePath, sha256: hash, dataBase64: data.toString('base64') },
            { relativePath, sha256: hash, dataBase64: data.toString('base64') },
          ],
        },
