...
async function makeTarget() {
  ...
  await writeFile(path.join(root, 'src', 'data', 'balance', 'species', 'generation-01.ts'), 'const generationOneSpeciesData: Record<SpeciesId, PokemonSpeciesData> = {};\nreturn generationOneSpeciesData;\n')
...
describe('authoring companion services', () => {
  it('detects a compatible arbitrary checkout and allocates unused stage IDs', async () => {
    ...
    expect(analysis.capabilities).toMatchObject({ species: true, eggMoves: true, encounters: true, sprites: true, cries: true, forms: false })
  ...
  it('stores and removes validated assets inside a portable project', async () => {
    ...
    expect(asset).toMatchObject({ kind: 'sprite', fileName: 'hero.png', size: data.length, width: 1, height: 1, mimeType: 'image/png' })
    expect(await readFile(path.join(created.projectDir, asset.relativePath))).toEqual(data)
    ...
    await expect(assets.save(created.projectDir, created.project.stages[0].stageId, 'sprite', {
      fileName: 'fake.png', size: 4, dataBase64: Buffer.from('nope').toString('base64'),
  ...
  it('runs preflight before install and journal validation before update', async () => {
    ...
    const runProcess = vi.fn(async (file, args) => {
      ...
      return { stdout: args.includes('--dry-run') ? 'preflight ok' : 'install ok', stderr: '' }
    })
    ...
    const updated = await service.apply(created.projectDir, target, { force: true })
    expect(updated.title).toMatch(/updated/i)

    expect(calls[0][1]).toEqual(expect.arrayContaining(['--manifest', '--dry-run']))
    expect(calls[1][1]).toEqual(expect.arrayContaining(['--manifest', '--dry-run']))
    expect(calls[2][1]).not.toContain('--dry-run')
    expect(calls[3][1]).toEqual(expect.arrayContaining(['--uninstall', 'delivery', '--dry-run']))
