...
function decodeBase64(value) {
  if (typeof value !== 'string' || !value.length) {
    throw requestError('Asset data is required.')
  }
  const normalized = value.replace(/\s+/g, '')
  const buffer = Buffer.from(normalized, 'base64')
  if (
    !buffer.length
    || buffer.toString('base64').replace(/=+$/g, '') !== normalized.replace(/=+$/g, '')
  ) {
    throw requestError('Asset data is not valid base64.')
  }
  if (buffer.length > MAX_ASSET_BYTES) {
    throw requestError(`Asset exceeds the ${MAX_ASSET_BYTES / 1024 / 1024} MB limit.`, 413)
  }
  return buffer
}
...
function validatePng(data) {
  if (!startsWithBytes(data, [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a])) {
    throw requestError('PNG asset has an invalid file signature.')
  }
  if (data.length < 24 || data.toString('ascii', 12, 16) !== 'IHDR') {
    throw requestError('PNG asset has no valid IHDR header.')
  }
  const width = data.readUInt32BE(16)
  const height = data.readUInt32BE(20)
  if (!width || !height || width > MAX_IMAGE_DIMENSION || height > MAX_IMAGE_DIMENSION) {
    throw requestError(`PNG dimensions must be between 1 and ${MAX_IMAGE_DIMENSION} pixels.`)
  }
...
function validateWebp(data) {
  if (
    ...
    throw requestError('WebP asset has an invalid file signature.')
  }
  return { mimeType: 'image/webp' }
}

function validateAudio(data, extension) {
  if (extension === '.ogg' && data.toString('ascii', 0, 4) !== 'OggS') {
    throw requestError('OGG cry has an invalid file signature.')
  }
  if (
    extension === '.wav'
    && (
    ...
    throw requestError('WAV cry has an invalid file signature.')
  }
  if (
    extension === '.mp3'
    ...
    throw requestError('MP3 cry has an invalid file signature.')
  }
  if (
    extension === '.m4a'
    && (data.length < 12 || data.toString('ascii', 4, 8) !== 'ftyp')
  ) {
    throw requestError('M4A cry has an invalid file signature.')
  }
  return {
    mimeType: {
      '.ogg': 'audio/ogg',
...
function validateVariant(data) {
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw requestError('Variant metadata must contain a JSON object.')
  }
  ...
  return {
    async save(projectDir, stageId, kind, file) {
      ...
      if (!EXTENSIONS[kind].has(extension)) {
        throw requestError(`Unsupported ${kind} file extension.`)
      }
      const data = decodeBase64(file?.dataBase64)
      if (Number(file?.size) && Number(file.size) !== data.length) {
        throw requestError('Asset size does not match the uploaded data.')
      }
      ...
      if (!isInside(canonicalAssets, target)) {
        throw requestError('Asset path escapes the project asset folder.')
      }
    ...
    async remove(projectDir, relativePath) {
      ...
      if (!isInside(canonicalAssets, canonicalFile) || canonicalFile === canonicalAssets) {
        throw requestError('Asset path escapes the project asset folder.')
      }
