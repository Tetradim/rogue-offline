  ...
  return (
    <div className="build-sheet">
      <div className="build-heading">
        <div><span className="panel-kicker">Custom stage authoring</span><h1>{stage.name}</h1><p>Author portable species data without modifying an official Pokémon.</p></div>
        <div className="stage-identity" title="Stage IDs remain stable when names and slugs change"><span>Immutable stage ID</span><code>{stage.stageId}</code></div>
      </div>
      ...
      <div className="build-grid">
        ...
        <section className="editor-card battle-card">
          ...
          <div className="field-triple">
            ...
            <label>Capture rate<input type="number" min="0" max="255" value={stage.captureRate} onChange={event => updateField('captureRate', Number(event.target.value))} /></label>
            <label>Male ratio %<input type="number" min="0" max="100" disabled={genderless} value={stage.genderRatio} onChange={event => updateField('genderRatio', Number(event.target.value))} /></label>
          </div>
          <fieldset className="flag-group">
            ...
            <label><input type="checkbox" checked={genderless} onChange={event => updateFlag('genderless', event.target.checked)} /> Genderless</label>
          </fieldset>
        </section>

        <section className="editor-card stats-card">
          <div className="card-heading"><div><h2>Base stats</h2><span className="card-subtitle">Slider and exact value stay synchronized</span></div><strong className="bst-pill">BST {calculateBst(stage)}</strong></div>
          <div className="stat-list">{STATS.map(([name, label]) => <StatSlider key={name} label={label} value={stage.baseStats[name]} onChange={value => onChange(setStageStat(project, stage.stageId, name, value))} />)}</div>
