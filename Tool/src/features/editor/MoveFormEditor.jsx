    ...
    <article className="form-editor">
      ...
      <fieldset className="form-stat-overrides">
        ...
        {STATS.map(([name, label]) => (
          <label key={name}>{label}<input type="number" min="1" max="255" value={stats[name] ?? ''} placeholder={String(stage.baseStats[name])} onChange={event => {
            const next = { ...stats }
  ...
  return (
    <div className="authoring-section">
      <div className="section-heading">
        ...
        <p>Use PokéRogue enum-style IDs; spaces are normalized automatically. Target Review identifies form features that a selected checkout cannot install safely.</p>
      </div>
      ...
      <section className="editor-card forms-card">
        <div className="card-heading">
          <div><h2>Alternate forms</h2><span className="card-subtitle">Identity, typing, abilities, passive, asset variant, and stat overrides</span></div>
